...
public class Fine {
	...
	public void displayFine() {
		...
		int borid;
		 ...
		 try{ stmt = con.createStatement(); rs =
		 stmt.executeQuery("SELECT * FROM Fine");
		 ...
		 System.out.printf("%-15s", rsmd.getColumnName(i+1));
		  ...
		  fid = rs.getInt("fine_fid"); System.out.printf("%-10.10s", fid);
		  
		 amount = rs.getFloat("fine_amount"); System.out.printf("%-20.20s",
		 amount);
		  
		 issueDate = rs.getDate("fine_issueDate");
		 System.out.printf("%-20.20s", issueDate);
		 
		 paidDate = rs.getDate("fine_paidDate"); System.out.printf("%-20.20s",
		 paidDate);
		 
		 borid = rs.getInt("borrowing_borid"); System.out.printf("%-20.20s",
		 borid);
	...
	public static void main(String[] args) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				//runFine.insertFine(11, 20, "2012-12-12", "2012-12-12", 20);
				runFine.deleteFine(10);
				System.out.print(runFine.checkFine(10));
				runFine.displayFine();
